...
if not exist "%~dp0CursorPos.exe" (
  echo Ну вот и все, ты вышел,
  echo пока-пока, возвращяйся еще.
  echo Больше проектов и игр-
  echo https://t.me/sterford_production
  echo(
...
set "desc1=Ленивец"
set "desc2=Чуть быстрее ленивца"
set "desc3=Медленно"
set "desc4=Нормальный"
set "desc5=Быстро"
set "desc6=Безумие"
...
set screen=!screen!Скорость = !Difficulty! !replay!!LF!Growth Rate = !growth!   Максимальный рекорд = !hi!!LF!Счет = !score!   Время = !m!:!s!%\n%
if defined replay if not defined replayFinished (%\n%
  set screen=!screen!!LF!!LF!Нажмите, чтобы остановить запись%\n%
)%\n%
%cls%^&echo(!screen!
...
for /l %%. in () do (
  ...
  if !tDiff! geq !delay! (
    ...
    if /i !key! equ !pause! (
      ...
      %=== pause game ===%
      echo(
      call :ask "Пауза - Нажмите чтобы продолжить
      %ClearPrev%
      %sendCmd% go
      for /f "tokens=1-4 delims=:.," %%a in ("!time: =0!") do set /a "t2=(((1%%a*60)+1%%b)*60+1%%c)*100+1%%d-36610100, tDiff=t2-t1"
      if !tDiff! lss 0 set /a tDiff+=24*60*60*100
      ...
      (%test% !X! !Y! playerSpace) && (
        ...
        echo(
        %ClearLine%
        call :ask "Нажмите, чтобы продолжить"
        for %%A in (!configOptions!) do set "%%A=!%%ASave!"
        call :mainMenu
      )
    )
...
echo(
echo   N - Новая игра
if defined replayAvailable echo   R - Предыдущие игры
if defined saveAvailable   echo   S - Сохранить игру
if exist *.snake.txt       echo   L - Посмотреть и включить сохраненную игру&set "loadAvailable=L"

echo   C - Настройки клавиш
echo   G - Графические настройки
echo   Q - Выйти
echo(

echo(
echo  Разработчик - NM_SterFord 
echo  Версия игры - 1.0.1  
echo(
set "hiAvailable="
for /l %%N in (1 1 6) do if defined hi%%N (
  if not defined hiAvailable (
    echo Запись лучшей игры:
    echo(
  )
  set "desc=!desc%%N!........"
...
if !moveKeys! equ 4 set "keys=!keys! U D"
                    echo Настройки клавиш:
                    echo(
                    echo   T - Type... = !moveKeys! keys
                    echo(
                    echo   L - Влево... = !left!
                    echo   R - Право.. = !right!
if !moveKeys! equ 4 echo   U - Выше..... = !up!
if !moveKeys! equ 4 echo   D - Ниже... = !down!
                    echo(
                    echo   P - Пауза.. = !pause!
                    echo(
                    echo   {Enter} - Обратно в главное меню
                    echo(
call :ask ">" !keys!
...
echo(
echo   B - Граниув...... = !bound!
echo   E - Пустота... = !space!
echo   H - Голова змеи.. = !head!
echo   S - Тело змеи.. = !body!
echo   F - Еда........ = !food!
echo   D - Смерть....... = !death!
echo(
echo   {Enter} - Обратно в главное меню
...
cls
if defined replay (
  echo Скорость записи:
) else (
  echo Настройки скорости:
)
echo                       Скорость
...
echo(
call :ask "Выберите скорость (1-6):" 1 2 3 4 5 6
...
if defined replay (
  ...
  %sendCmd% !replay!
  call :waitForSignal
  set "replay=(Запись от !difficulty!)"
  set "read=1"
  <&9 (
